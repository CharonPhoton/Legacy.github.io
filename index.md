...
|행동|영문명칭|설명|
|:----:|:----:|:----:|
|공격|Move_Up|플레이어가 ↑키를 누르면 위 레일로 이동한다.|
|능력 사용|Move_Down|플레이어가 ↓키를 누르면 아래 레일로 이동한다.|
|아이템 사용|Attack|플레이어가 Space키를 누르면 화살을 자신의 위치에서 소환.|
|사망|Item_Use1|플레이어가 1키를 누르면 아이템 하나를 소비하여 체력을 하나 회복.|
