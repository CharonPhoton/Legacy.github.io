...
### 1) 스테이지에 사용되는 공식
 ...
 - 보상이 최소, 최댓값이 존재할 경우, 그 사이의 난수를 계산하여 플레이어에게 지급한다.
...
- 플레이어 캐릭터의 체력과 에너지는 모두 각 캐릭터의 UI 아래에서 표시한다.
- 적의 레벨, 이름은 각 적 오브젝트 이미지 상 위쪽에 표시하고, 체력은 아래쪽에 표시한다.
...
- 데이터 저장은 모두 json 파일로 저장하며, 저장한 json 파일을 불러오도록 한다.
- 카드와 분대원 보유 데이터 저장은 각각의 데이터 파일을 별도로 사용해서 저장 및 불러오도록 한다.
- 
### 플레이어 관련 - 아이템 인벤토리
...
# [ 저장 데이터 세부 분선 ]<a name='9'></a>

1) json 데이터 : Charactor, Squad, Card

|데이터 명|설명|
|:----:|:----:|
|index|해당 분대원 또는 카드의 위치|
|Career|해당 분대원의 직업|
|DetailCareer|해당 분대원의 전직 직업|
|Level|해당 분대원의 레벨|
|EXP|해당 분대원의 경험치|
|MaxHP|해당 분대원의 최대 체력|
|BaseDamage|해당 분대원의 공격력|
|BaseSpeed|해당 분대원의 공격 속도|
|MaxMana|해당 분대원의 최대 마나|
|Skill|해당 분대원이 장착하고 있는 스킬|
|item|해당 분대원이 장착하고 있는 아이템|
|item_Duration|해당 분대원이 장착하고 있는 아이템의 내구도|
|equipment|해당 분대원이 장착하고 있는 장비 아이템|
|equipment_Duration|해당 분대원이 장착하고 있는 장비 아이템의 내구도|
|Critical|해당 분대원의 크리티컬 데미지|
|Critical_Pro|해당 분대원의 크리티컬 확률|
|HP_Upgrade|해당 분대원의 현재 체력 업그레이드 투자 수|
|Damage_Upgrade|해당 분대원의 현재 데미지 업그레이드 투자 수|
|AttackSpeed_Upgrade|해당 분대원의 현재 공격 속도 업그레이드 투자 수|
|Energy_Upgrade|해당 분대원의 현재 에너지 업그레이드 투자 수|
|Critical_Upgrade|해당 분대원의 현재 크리티컬 데미지 업그레이드 투자 수|
|Critical_Pro_Upgrade|해당 분대원의 현재 크리티컬 확률 업그레이드 투자 수|
|nulled|현재 캐릭터가 빈 것인지 확인하는 bool 변수|

2) json 데이터 : Item

|데이터 명|설명|
|:----:|:----:|
|index|딕셔너리 변수를 사용할 때 사용될 해당 아이템의 Key값|
|Type|해당 아이템의 ID|
|Duration|해당 아이템의 내구도(만약 재료 아이템일 경우 갯수로 사용된다.)|
...
|ID|해당 아이템의 ID|
|Name|해당 아이템의 이름|
|Duration|해당 아이템의 본래 내구도 (만약 재료 아이템일 경우 10 또는 20으로 되어있으며, 아무 숫자로 저장해도 상관없다.)|
|Information|해당 아이템의 스토리적 설명|
|Effect|해당 아이템의 효과|
...
|:----:|:----:|
|None|빈 곳인지 판별하는 스트링|
|Stage Start|한 스테이지 데이터를 불러올 준비|
|Unit|스테이지에 나오는 적들을 불러올 준비|
|Round|1~20까지 어떤 위치에서 적이 나오는지 불러온다. Round가 다음 줄에 있으면, 2 라운드의 적들의 정보를 그 다음 줄의 Round에서 불러온다.|
|RewardItem|보상 아이템이 있는지 없는지 판별한다.|
|Item|보상 아이템이 있으면 4칸씩 데이터를 불러온다. 각 데이터는 1 : 아이템의 ID, 2 : 해당 아이템의 드랍 확률, 3 : 최소값, 4 : 최댓값이다.|
|RewardEquip|보상 장비 아이템이 있는지 없는지 판별한다.|
|Equip|보상 장비 아이템이 있으면 4칸씩 데이터를 불러온다. 각 데이터는 1 : 아이템의 ID, 2 : 해당 아이템의 드랍 확률, 3 : 최소값, 4 : 최댓값이다.|
|RewardMaterial|보상 재료 아이템이 있는지 없는지 판별한다.|
|Material|보상 재료 아이템이 있으면 4칸씩 데이터를 불러온다. 각 데이터는 1 : 아이템의 ID, 2 : 해당 아이템의 드랍 확률, 3 : 최소값, 4 : 최댓값이다.|
|EXP|클리어 시 얻을 분대원들이 받을 경험치|
|UserEXP|클리어 시 얻을 플레이어의 경험치|
|Coin|클리어 시 얻을 재화. 2칸으로 되어 있으며, 1 : 최소값, 2 : 최댓값이다.|
|Stage End|한 스테이지의 데이터를 불러오는 것을 종료한다.|
...
- ~~데이터 저장은 모두 json 파일로 저장하며, 저장한 json 파일을 불러오도록 한다. (100%)~~
- ~~카드와 분대원 보유 데이터 저장은 각각의 데이터 파일을 별도로 사용해서 저장 및 불러오도록 한다. (100%)~~
...
- ### 13) 13주 - 스테이지 제작 : 한 스테이지의의 진행 및 결과 이벤트 발생

- ~~한 스테이지에 등장하는 무리들, 그리고 한 무리 당 어떤 적들이 있는지 csv파일로 저장하고 게임에서 불러오게 한다. (100%)~~
- ~~적 무리를 처치하면 다음 적 무리가 있으면, 그 적 무리와 전투를 실행하도록 한다. (100%)~~
- ~~적 무리를 모두 처치하면 게임에서 승리했다는 UI를 출력한다. 그 후, 스테이지에 참전한 분대원 캐릭터들에게 경험치를 나누어준다. (100%)~~
- ~~적 무리와 싸우다가 플레이어의 캐릭터가 전멸하면 게임에서 패배했다는 UI를 출력한다. (100%)~~
- ~~스테이지 진행도는 화면 중앙 상단에 표시된다. (100%)~~
- ~~스테이지의 정보는 모두 DataSystem이라는 스크립트에서 Dictionary로 사용해 담아둔다. (100%)~~
- ~~스테이지의 정보는 게임을 시작함과 동시에 저장하도록 만든다. (100%)~~
- ~~스테이지 정보 csv파일에서 첫번째 스트링이 Stage Start라면, Stage의 정보를 저장할 준비를 한다. (데이터 구조체 클래스 한 개, 딕셔너리를 담는 리스트 한 개, 아이템 리스트 세 개를 모두 초기화한다.) (100%)~~
- ~~스테이지 정보 csv파일에서 첫번째 스트링이 Stage라면, 이후 20개의 스트링에서 유닛이 어디에 배치 되는지 불러온다. 이때, 이 정보는 Dictionary 형식으로 저장한다.(key는 값이 1 이상인 곳인 위치, value는 해당 값) (100%)~~
- ~~스테이지 정보 csv파일에서 첫번째 스트링이 RewardItem라면, 그 다음 스트링이 False인지 True인지 확인한다. (100%)~~
- ~~만약 RewardItem 다음이 false라면 넘어가고, true인 경우에는 그 다음 줄인 Item에서 네 개 단위의 스트링을 받는다. (1 : 보상으로 나올 아이템 ID, 2 : 나올 확률, 3 : 최소값, 4 : 최댓값) (100%)~~
- ~~스테이지 정보 csv파일에서 첫번째 스트링이 RewardEquip라면, 그 다음 스트링이 False인지 True인지 확인한다. (100%)~~
- ~~만약 RewardEquip 다음이 false라면 넘어가고, true인 경우에는 그 다음 줄인 Equip에서 네 개 단위의 스트링을 받는다. (1 : 보상으로 나올 아이템 ID, 2 : 나올 확률, 3 : 최소값, 4 : 최댓값) (100%)~~
- ~~스테이지 정보 csv파일에서 첫번째 스트링이 RewardMaterial라면, 그 다음 스트링이 False인지 True인지 확인한다. (100%)~~
- ~~만약 RewardMaterial 다음이 false라면 넘어가고, true인 경우에는 그 다음 줄인 Material에서 네 개 단위의 스트링을 받는다. (1 : 보상으로 나올 아이템 ID, 2 : 나올 확률, 3 : 최소값, 4 : 최댓값) (100%)~~
- ~~보상 Item, Equip, Material에서 나올 결과는 각각 List로 저장한다(네 개 단위로 받은 스트링은 Class 구조체 형식으로 저장한다.). (100%)~~
- ~~스테이지 정보 csv파일에서 첫번째 스트링이 EXP이라면, 그 다음 스트링에 있는 숫자를 변수로 저장한다. (100%)~~
- ~~스테이지 정보 csv파일에서 첫번째 스트링이 UserEXP이라면, 그 다음 스트링에 있는 숫자를 변수로 저장한다. (100%)~~
- ~~스테이지 정보 csv파일에서 첫번째 스트링이 Coin이라면, 그 다음 두 스트링에 있는 숫자를 변수로 저장한다. (100%)~~
- ~~StageEnd가 되면, 저장된 List들과 변수들을 모두 Dictionary 형식으로 저장한다. (Key는 StageStart가 될 때 증가 하는 a의 값, Value는 각 리스트들을 포함하는 클래스 구조체) (100%)~~
- ~~만들어진 Dictionary 변수를 DataSystem으로 반환하고, 해당 스테이지를 시작하면 DataSystem에 저장된 해당 스테이지의 정보를 불러오게 한다. (100%)~~
- ~~적 정보는 csv로 불러와 Dictionary로 저장한다. ( 이때, 레벨, 체력, 공격 속도, 데미지, 스킬 순으로 저장한다. ) (100%)~~
- ~~적 정보 데이터도 DataSystem에서 딕셔너리 형식으로 저장한다. (100%)~~
- ~~카드나 분대원의 정보 창에서 이전 및 다음 버튼을 누르면 창이 꺼지는 현상 수정 -> 창을 끄는 코드가 추가 되어 있었는데 이것을 지웠다. (100%)~~
- ~~적의 레벨, 이름은 각 적 오브젝트 이미지 상 위쪽에 표시하고, 체력은 아래쪽에 표시한다. (100%)~~
