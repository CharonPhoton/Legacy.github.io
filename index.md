...
- 인벤토리 칸은 한 줄에 최대 5칸이 뜬다.
- 저장 방식을 바꾼다. ( 이전 : 베이스 데이터 파일, 세이브 데이터 파일을 불러와서 세이브 데이터 파일을 베이스 데이터 파일에 덮어씌운다. -> 변경 : 세이브 데이터 파일만을 불러서 적용한다. )[ 이것은 이후에 하게 될 수도 있다. ]
